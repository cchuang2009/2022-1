...
cat_f = ['Company', 'TypeName', 'Cpu_brand',
         'GPU', 'first_type', 'second_type', 'os']
...
if Company == 'Apple':
    os = 'Mac'
    os_of_laptop = 0
    st.markdown("**Operation System**:  Mac")
else:
    # from df['os'].unique() remove Mac

    ops = ['Others/No OS/Linux', 'Windows']
    # ops=list(df["os"].unique()).remove('Mac')
    #os = st.selectbox("Operation System", options=df["os"].unique())
    os = st.selectbox("Operation System", options=ops)
...
second_type = st.selectbox(
    "Second Storage", options=df["second_type"].unique())
...
size_in_inches = st.number_input(
    'Size of the laptop in Inches', step=0.1, min_value=10.0)
...
first_size = st.slider('Size of First Storage',
                       min_value=128, max_value=512, value=128,step=64)
